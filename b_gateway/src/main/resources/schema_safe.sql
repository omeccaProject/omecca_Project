-- omecca DB 스키마 (target / roi / event / report / user / camera / camera_catalog)
-- schema.sql과 내용은 동일하지만, DROP TABLE을 전부 뺀 안전 버전입니다.
-- 이미 데이터가 들어있는 DB에 그대로 실행해도 기존 테이블/데이터를 지우지 않고,
-- 없는 테이블만 새로 만들고 없는 행만 추가합니다 (CREATE TABLE IF NOT EXISTS + INSERT IGNORE).
--
-- 사용법 (Windows PowerShell):
--   Get-Content src/main/resources/schema_safe.sql | mysql -u root -p
-- 사용법 (Mac/Linux):
--   mysql -u root -p < src/main/resources/schema_safe.sql
...
-- (팀원 모듈이 여기 등록 안 된 cam_id를 보내면 이벤트 저장 자체가 막히기 때문).
...
INSERT IGNORE INTO user (username, password, name, role, status, approved_at)
